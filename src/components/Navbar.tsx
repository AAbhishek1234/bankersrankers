...
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import logo from "@/assets/bankerslogo.png";
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md">
      <nav className="container mx-auto flex items-center justify-between py-4">
          ...
          {navLinks.map((link) => (
            <li key={link.name}>
              <Link
                to={link.path}
                className={cn(
                  "relative font-medium text-foreground transition-colors py-2",
                  "after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 after:bg-secondary after:transition-all hover:after:w-full",
                  location.pathname === link.path &&
                    "text-primary after:w-full"
                )}
              >
        ...
        <div
          className={cn(
            "absolute top-full left-0 right-0 bg-white shadow-md md:hidden transition-all overflow-hidden",
            isOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
          )}
        >
          <ul className="flex flex-col p-4 gap-2">
